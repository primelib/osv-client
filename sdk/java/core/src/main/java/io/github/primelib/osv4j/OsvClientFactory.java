...
import io.github.primelib.osv4j.client.OsvClientApi;
import io.github.primelib.osv4j.client.OsvClientConsumerApi;
...
public class OsvClientFactory {
    public static <T> T create(Consumer<OsvClientFactorySpec<T>> spec) {
        OsvClientFactorySpec<T> config = new OsvClientFactorySpec<>(spec);

        if (config.api().isInterface()) {
            return buildClient(spec);
        }
        if (config.api() == OsvClientConsumerApi.class) {
            return (T) OsvClientFactory.buildConsumerClient(OsvClientConsumerApi.class, OsvClientApi.class, (OsvClientFactorySpec<OsvClientConsumerApi>) config);
        }

        throw new IllegalArgumentException("api must be an compatible interface or consumer class");
    }

    public static OsvClientApi create() {
        return create(spec -> spec.api(OsvClientApi.class));
    }

    private static <T, U> T buildConsumerClient(Class<T> consumerApiClass, Class<U> apiClass, OsvClientFactorySpec<T> config) {
        try {
            U api = buildClient(s -> {
                s.api(apiClass);
                s.applySpec(config);
    ...
    private static <T> T buildClient(Consumer<OsvClientFactorySpec<T>> spec) {
        OsvClientFactorySpec<T> config = new OsvClientFactorySpec<>(spec);
